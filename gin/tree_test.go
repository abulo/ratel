...
func TestTreeAddAndGet(t *testing.T) {
	...
	for _, route := range routes {
		tree.addRoute(route, util.Uniqid(""), fakeHandler(route))
	}
...
func TestTreeWildcard(t *testing.T) {
	...
	checkRequests(t, tree, testRequests{
		...
		// fix PR: https://github.com/abulo/ratel/v3/gin/pull/2796
...
func testRoutes(t *testing.T, routes []testRoute) {
	...
	for _, route := range routes {
		recv := catchPanic(func() {
			tree.addRoute(route.path, util.Uniqid(""), nil)
		})
...
func TestTreeDuplicatePath(t *testing.T) {
	...
	for _, route := range routes {
		...
		// Add again
		recv = catchPanic(func() {
			tree.addRoute(route, util.Uniqid(""), nil)
		})
		if recv == nil {
...
func TestTreeRootTrailingSlashRedirect(t *testing.T) {
	...
	recv := catchPanic(func() {
		tree.addRoute("/:test", util.Uniqid(""), fakeHandler("/:test"))
	})
	if recv != nil {
...
func TestTreeInvalidNodeType(t *testing.T) {
	...
	tree := &node{}
	tree.addRoute("/", util.Uniqid(""), fakeHandler("/"))
	tree.addRoute("/:page", util.Uniqid(""), fakeHandler("/:page"))
...
func TestTreeWildcardConflictEx(t *testing.T) {
	...
	for _, conflict := range conflicts {
		...
		recv := catchPanic(func() {
			tree.addRoute(conflict.route, util.Uniqid(""), fakeHandler(conflict.route))
		})
