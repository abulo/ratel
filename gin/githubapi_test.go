...
func TestShouldBindUri(t *testing.T) {
	...
	router.Handle(http.MethodGet, "/rest/:name/:id", util.Uniqid(""), func(c *Context) {
		var person Person
...
func TestBindUriError(t *testing.T) {
	...
	router.Handle(http.MethodGet, "/new/rest/:num", util.Uniqid(""), func(c *Context) {
		var m Member
...
func TestRaceContextCopy(t *testing.T) {
	DefaultWriter = os.Stdout
	router := Default()
	router.GET("/test/copy/race", util.Uniqid(""), func(c *Context) {
		c.Set("1", 0)
		c.Set("2", 0)
...
func githubConfigRouter(router *Engine) {
	for _, route := range githubAPI {
		router.Handle(route.method, route.path, util.Uniqid(""), func(c *Context) {
