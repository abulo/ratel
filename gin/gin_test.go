...
func setupHTMLFiles(t *testing.T, mode string, tls bool, loadMethod func(*Engine)) *httptest.Server {
	...
	captureOutput(t, func() {
		...
		loadMethod(router)
		router.GET("/test", util.Uniqid(""), func(c *Context) {
			c.HTML(http.StatusOK, "hello.tmpl", map[string]string{"name": "world"})
		})
		router.GET("/raw", util.Uniqid(""), func(c *Context) {
...
func TestH2c(t *testing.T) {
	...
	r := Default()
	r.UseH2C = true
	r.GET("/", util.Uniqid(""), func(c *Context) {
...
func TestAddRoute(t *testing.T) {
	router := New()
	router.addRoute("GET", "/", util.Uniqid(""), HandlersChain{func(_ *Context) {}})

	assert.Len(t, router.trees, 1)
	assert.NotNil(t, router.trees.get("GET"))
	assert.Nil(t, router.trees.get("POST"))

	router.addRoute("POST", "/", util.Uniqid(""), HandlersChain{func(_ *Context) {}})

	assert.Len(t, router.trees, 2)
	assert.NotNil(t, router.trees.get("GET"))
	assert.NotNil(t, router.trees.get("POST"))

	router.addRoute("POST", "/post", util.Uniqid(""), HandlersChain{func(_ *Context) {}})
	assert.Len(t, router.trees, 2)
}

func TestAddRouteFails(t *testing.T) {
	router := New()
	assert.Panics(t, func() { router.addRoute("", "/", util.Uniqid(""), HandlersChain{func(_ *Context) {}}) })
	assert.Panics(t, func() { router.addRoute("GET", "a", util.Uniqid(""), HandlersChain{func(_ *Context) {}}) })
	assert.Panics(t, func() { router.addRoute("GET", "/", util.Uniqid(""), HandlersChain{}) })
...
func TestListOfRoutes(t *testing.T) {
	router := New()
	router.GET("/favicon.ico", util.Uniqid(""), handlerTest1)
	router.GET("/", util.Uniqid(""), handlerTest1)
	group := router.Group("/users")
	{
		group.GET("/", util.Uniqid(""), handlerTest2)
		group.GET("/:id", util.Uniqid(""), handlerTest1)
		group.POST("/:id", util.Uniqid(""), handlerTest2)
	}
	...
	assertRoutePresent(t, list, RouteInfo{
		Method:  "GET",
		Path:    "/favicon.ico",
		Handler: "^(.*/vendor/)?github.com/abulo/ratel/v3/gin.handlerTest1$",
	})
	assertRoutePresent(t, list, RouteInfo{
		Method:  "GET",
		Path:    "/",
	...
	assertRoutePresent(t, list, RouteInfo{
		Method:  "GET",
		Path:    "/users/",
		Handler: "^(.*/vendor/)?github.com/abulo/ratel/v3/gin.handlerTest2$",
	})
	assertRoutePresent(t, list, RouteInfo{
		Method:  "GET",
		Path:    "/users/:id",
...
func TestEngineHandleContext(t *testing.T) {
	...
	v2 := r.Group("/v2")
	{
		v2.GET("/", util.Uniqid(""), func(c *Context) {})
	}
...
func TestEngineHandleContextManyReEntries(t *testing.T) {
	...
	r.GET("/:count", util.Uniqid(""), func(c *Context) {
