...
func TestWrap(t *testing.T) {
	router := New()
	router.POST("/path", util.Uniqid(""), WrapH(&testStruct{t}))
	router.GET("/path2", util.Uniqid(""), WrapF(func(w http.ResponseWriter, req *http.Request) {
...
func TestFunctionName(t *testing.T) {
	assert.Regexp(t, `^(.*/vendor/)?github.com/abulo/ratel/v3/gin.somefunction$`, nameOfFunction(somefunction))
}
...
func TestBindMiddleware(t *testing.T) {
	...
	var called bool
	router := New()
	router.GET("/", util.Uniqid(""), Bind(bindTestStruct{}), func(c *Context) {
		called = true
