...
func (r *Client) Exists(ctx context.Context, key ...string) (val int64, err error) {
	err = r.brk.DoWithAcceptable(func() error {
		conn, err := getRedis(r)
		if err != nil {
			return err
		}
		val, err = conn.Exists(r.getCtx(ctx), r.ks(key...)...).Result()
		return err
	}, acceptable)
	return
}
...
// func (r *Client) LSet(ctx context.Context, key string, index int64, value any) *redis.StatusCmd {
...
// func (r *Client) SIsMember(ctx context.Context, key string, member any) *redis.BoolCmd {
...
// func (r *Client) SMove(ctx context.Context, source, destination string, member any) *redis.BoolCmd {
...
// // Eval 执行 Lua 脚本。
// func (r *Client) Eval(ctx context.Context, script string, keys []string, args ...any) *redis.Cmd {
// 	return getRedis(r).Eval(r.getCtx(ctx), script, r.ks(keys...), args...)
// }

// // EvalSha 执行 Lua 脚本。
// func (r *Client) EvalSha(ctx context.Context, sha1 string, keys []string, args ...any) *redis.Cmd {
// 	return getRedis(r).EvalSha(r.getCtx(ctx), sha1, r.ks(keys...), args...)
// }

// // EvalRO 执行 Lua 脚本。
// func (r *Client) EvalRO(ctx context.Context, script string, keys []string, args ...any) *redis.Cmd {
// 	return getRedis(r).EvalRO(r.getCtx(ctx), script, r.ks(keys...), args...)
// }

// // EvalShaRO 执行 Lua 脚本。
// func (r *Client) EvalShaRO(ctx context.Context, sha1 string, keys []string, args ...any) *redis.Cmd {
